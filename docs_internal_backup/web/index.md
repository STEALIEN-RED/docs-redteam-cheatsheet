...
웹은 레드팀이 외부에서 내부로 들어가는 가장 흔한 입구. 공개되어 있고, 인증 전 단계의 endpoint 가 존재하고, 개발스러운 코드가 많기 때문.

심지어 내부망 충분전이 끝난 뒤에도 내부용 portal / admin panel 이 pivot 지점이 되는 경우가 굉장히 많다.
...
sqlmap -u '<url>?id=1' -D <db> -T <tbl> --dump  # 데이터 dump
...
- Try-It-Out proxy: Origin 검증 부재 시 CSRF/SSRF 연계
...
1) endpoint 탐지 → 2) 스키마 dump 후 Postman/Insomnia 가져오기 → 3) 권한/경계 값 fuzzing → 4) 멱등성/RateLimit 검증 → 5) 비즈니스 로직 테스트
...
# Apollo Persisted Queries 우회: APQ hash 강제 미사용
{"query":"...","extensions":{}}
```
...
# Webhook / file upload mutation → SSRF / 임의 파일 upload
```
...
- [graphqlmap](https://github.com/swisskyrepo/GraphQLmap) — 인터랙티브 exploit
...
- code 가 redirect_uri/client_id 와 binding되는지 (PKCE code_verifier)
...
캐시 서빙 레이어(proxy/CDN)와 오리진 간 parsing 차이를 이용해 악성 응답을 캐시에 주입하거나 민감 응답을 캐시에 저장하게 만드는 기법.
