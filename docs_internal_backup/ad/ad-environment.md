...
그래서 항상 도메인의 현재 상태를 먼저 파악한 다음 공격 벡터를 정하는 편이 안전하고 결과도 훨씬 깔끔하게 갈린다.
...
!!! info "관련 문서"
    프로토콜 자체 설명(NetNTLMv1/v2, hash 유형, 보안 통제 매트릭스, 탐지 관점)은 [NTLM](../protocols/ntlm.md) 문서 참고. 강제 인증(Coercion) 기법은 [NTLM Coercion](coercion.md) 참고.
