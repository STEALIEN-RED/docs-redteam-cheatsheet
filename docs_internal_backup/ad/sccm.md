...
`Client Push Installation` 이 활성화되어 있으면 SCCM 이 새 호스트에 에이전트를 설치하기 위해 **Client Push Account credential**을 보낸다. 이를 NTLM 으로 캡처/릴레이.
...
캡처된 credential은 종종 **로컬 관리자 권한이 광범위하게 부여**되어 있음 → 즉시 PtH 로 횡적 이동.
