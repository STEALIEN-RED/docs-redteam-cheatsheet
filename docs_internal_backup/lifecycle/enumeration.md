...
초기 침투 후 내부 네트워크에서 사용자, 그룹, 공유 폴더, 서비스 등의 정보를 수집하는 단계.

공격에 앞서 현재 환경에 대한 이해를 높이고, 공격 경로를 결정하기 위해 반드시 수행한다.
...
```bash
# 기본 LDAP 질의 (익명 바인딩, -x 옵션은 Simple Authentication)
...
# 패스워드 정책 확인 (lockout threshold, 최소 길이 등)
...
Python 기반 LDAP 열거 도구. 익명 바인딩이 허용된 환경에서 유용하다.
...
rpcclient $> getdompwinfo      # 패스워드 정책
```
...
수집된 JSON/ZIP 파일을 BloodHound GUI에 업로드하면 공격 경로 분석이 가능하다.
