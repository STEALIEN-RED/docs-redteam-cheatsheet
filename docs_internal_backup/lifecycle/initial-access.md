...
외부에서 내부 경계를 떫는 지점이다. 피싱으로 사람 실수를 노리거나, 노출된 외부 서비스의 취약점을 이용하거나, 약한 credential 을 뚫어서 최초 접근을 만든다.

대부분의 레드팀 scope 에서 변수가 가장 많은 단계다. OPSEC 이 무너지면 뒤에 연결된 공격이 모두 문힌다보니 조심스럽게 진행한다.
...
레드팀 작전에서 초기 침투 시 가장 많이 사용되는 기법 중 하나. 상세는 [Phishing / Vishing](../techniques/phishing.md) 참고.
