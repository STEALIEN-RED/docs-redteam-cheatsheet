...
- [ ] **리다이렉터**: C2 서버를 직접 노출하지 말고 Apache/Nginx/Caddy 기반 리다이렉터를 앞단에 배치. Cobalt Strike Malleable Profile / Sliver HTTPS Profile 로 트래픽 패턴 은닉
...
- [ ] **C2 프로파일**: 기본 프로파일 그대로 사용 금지. User-Agent / URI / 헤더 순서 / JA3 모두 수정

### 페이로드

- [ ] 빌드 직후 사내 샌드박스 또는 `AntiScan.me` / `KleenScan` 로 탐지율 확인 (VirusTotal 은 공유되므로 **업로드 금지**)
- [ ] 코드 서명 인증서 + 타임스탬프 + 리소스 정보(아이콘/버전) 삽입
- [ ] 지연 실행(sleep jitter), Sandbox 탐지(VM 아티팩트 / 사용자 상호작용)
- [ ] 도메인 / 프로세스 / 사용자 / 시간 제약 조건으로 **타겟 환경에서만 동작**하도록 제한

### 트래픽

- [ ] **비콘 지터** 최소 20% 이상 랜덤화
- [ ] 타겟의 **업무 시간대**에만 체크인 하도록 스케줄
...
| 중소 인게이지먼트 | Sliver + Caddy 리다이렉터 + Ligolo-ng |
| 대규모 / 장기 인게이지먼트 | Cobalt Strike / Mythic + Malleable C2 + 다단 리다이렉터 |
...
- 각 단계별 세션/TTP/타임스탬프를 **문서화**하여 디브리핑 및 탐지 매핑(ATT&CK)에 활용
- 세부 도구/페이로드별 사용법은 하위 페이지 참고
